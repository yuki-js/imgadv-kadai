...
マウスオーバーした点に対応する 48x48x1 (grayscale) 画像を右側パネルに表示する。
...
def _vec_to_gray_image(v: np.ndarray, *, h: int = 48, w: int = 48) -> np.ndarray:
    """(D,) -> (H,W) in [0,1] (min-max per image)."""

    img = np.asarray(v, dtype=np.float64).reshape(h, w)
    vmin = float(img.min())
    vmax = float(img.max())
...
def main() -> None:
    ...
    imgs = np.stack([_vec_to_gray_image(v) for v in imgs_vec], axis=0)  # (N,48,48)
    ...
    ax.set_title(
        "Query4 (mpl): Projection onto difference subspace\n"
        f"eig={np.array2string(evals, precision=4)}"
    )
    ax.set_xlabel("ds-1")
    ax.set_ylabel("ds-2")
    ax.set_zlabel("ds-3")
    ...
    im_artist = ax_img.imshow(
        np.zeros((48, 48), dtype=np.float64),
        cmap="gray",
        vmin=0.0,
        vmax=1.0,
    )
